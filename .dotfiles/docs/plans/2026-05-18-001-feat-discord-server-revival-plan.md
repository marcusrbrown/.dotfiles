...
- Document the bot-token injection method — `DISCORD_TOKEN` env var passed directly into the container (no `*_FILE` indirection support in SaseQ/discord-mcp). Token sourcing options on this machine (in order of preference): (a) macOS Keychain via `security find-generic-password -s discord-bot-fro-bot -w` invoked from `~/.zshrc.local` (gitignored); (b) plaintext export in `~/.zshrc.local` (gitignored, machine-local); (c) future option: 1Password CLI via `op read` once `op` is installed and added to the Brewfile. Token MUST NOT land in any tracked dotfiles content.
  - **Superseded — do not follow this bullet's sourcing detail.** `.config/bash/local.d/` was never sourced by any shell and has since been removed; `~/.zshrc.local` is the supported machine-local hook. The credentials live in two keychain entries (`discord-bot-fro-bot-token`, `discord-bot-fro-bot-guild-id`), not one. `docs/runbooks/discord-admin-agent.md` is authoritative. The single-entry name above is what produced a duplicate token under an unread service name; see PRs #2500 and #2501.
