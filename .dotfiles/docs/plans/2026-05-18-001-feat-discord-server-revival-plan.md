...
- Determine transport: HTTP (`http://localhost:8085/mcp`, the recommended/default per `SaseQ/discord-mcp` README) OR stdio (legacy/local container mode). Default: **HTTP**, allows reuse across sessions and matches upstream recommendation.
- **Preflight check:** Before committing to HTTP, verify Docker is available, port 8085 is bindable, and `http://localhost:8085/mcp` is reachable from the OpenCode session. If preflight fails, fall back to stdio (which keeps the same admin-agent capabilities, just without cross-session reuse). Document the preflight commands in the runbook.
- Pin via Docker image digest (`saseq/discord-mcp@sha256:...`) or commit SHA tag.
- Document the bot-token injection method — token comes from a 1Password vault read or local secret file, never tracked.
