...
- R12. Privileged intents are enabled in the Developer Portal AND declared in code only as needed: `MessageContent` only if the gateway needs to read non-mention message content; `Server Members` only if member-tracking features are used. Production-server activation of `MessageContent` is gated by R13/R14 (disclosure-before-intent); a staging/sandbox server with a synthetic disclosure placeholder MAY be used to test the gateway's behavior with `MessageContent` enabled before the production disclosure lands.

**AI disclosure + governance (Phase 1.5, BEFORE Phase 2 goes live)**

- R13. A pinned disclosure document exists in the server before the bot goes live with `MessageContent` intent, covering: bots operating in the server, intents/data accessed, data retention windows, third-party processors (if any), appeals process for moderation actions, AND the admin-agent's own message-body access during audit/restructure operations (what it reads, what it archives, where archives live, how long they're retained).
...
**AE3 — Covers R12, R13, R14** (behavioral-conditional: disclosure-before-intent):
> *When* the gateway daemon first runs in the server with `MessageContent` intent enabled, *then* the pinned disclosure document already exists in a public-readable channel and references the specific intents being used. The `MessageContent` intent in R12 MUST NOT be enabled in production until R13/R14 are satisfied.
